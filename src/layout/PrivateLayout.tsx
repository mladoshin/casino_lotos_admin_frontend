...
useEffect(()=>{
  if (isAsideOpen)document.body.classList.add('noscroll'); else document.body.classList.remove('noscroll');
},[isAsideOpen]);
  useEffect(() => {
...
for (var d = 0; d < menuItems[i].children.length; d++){
 if ( menuItems[i].children[d].key == pn ) return menuItems[i].children[d].label;
}
    }else if (menuItems[i].key == pn )
        ...
        <Content className="main-c" style={{ padding: "48px" }}>
          ...
          >
            <Sider onClick={(e)=>{
              ...
              if(e.target?.nodeName == 'LI' || e.target != null && e.target.nodeName == 'SPAN' ){
                setAsideOpen(false);
              }
            }} className={isAsideOpen ? 'aside--open' : ''} style={{ background: colorBgContainer }} width={250}>
              <Menu
                mode="inline"
